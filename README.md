...
## v0.1.5 EU resilient notes

This package is configured for Render's Frankfurt region (`region: frankfurt`) and includes a safer CIE start flow. If BDP does not reach `DOMContentLoaded`, the app no longer fails the whole request immediately; it returns a screenshot and warning so you can refresh or navigate manually.

After logging into the prototype, these diagnostic endpoints are useful:

- `/api/debug/browser` confirms that cloud Chromium can launch and screenshot.
- `/api/debug/bdp-network` confirms that the Render service can fetch `https://bdp.giustizia.it/` from the cloud network.

If `/api/debug/browser` works but `/api/debug/bdp-network` fails, the issue is network reachability from Render to BDP rather than Playwright itself.
